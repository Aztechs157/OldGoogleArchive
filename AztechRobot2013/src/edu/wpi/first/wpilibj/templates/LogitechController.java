...
//From these calculate: 
        ...
        if ((JsValue < JsDeadbandMidLimitUpper) && (JsValue > JsDeadbandMidLimitLower)) {
            return JsScaled = DesiredOutputValueMid;
        } //in middl deadband output mid value 
        else if (JsValue > JsDeadbandMaxLimit) {
            return JsScaled = DesiredOutputValueMax;
        } //in upper deadband output max value 
        else if (JsValue < JsDeadbandMinLimit) {
            return JsScaled = DesiredOutputValueMin;
        } //in lower deadband output min value 
        else if (JsValue >= JsDeadbandMidLimitUpper) {

            return JsScaled = DesiredOutputValueMid + OutputRangeUpper * (JsValue - JsDeadbandMidLimitUpper) / JsUpperLinearRange;
        } //in upper linear region 
